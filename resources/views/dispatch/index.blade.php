...
@section('title') Dispatched Items @endsection
        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-start mb-3">
                    <div>
                        <h5 class="mb-0">All Dispatched Items</h5>
                    </div>
                </div>
                        ...
                        @can('add-item')
                            <div class="mt-3 mt-md-0 pt-5 pt-md-0">
                                <a href="javascript:void(0);">
                                    <button class="btn btn-primary mt-4" data-bs-target="#dispatchModal" id="addDispatch">Add Purchase Item</button>
                                </a>
                            </div>
                        @endcan
                    </div>
...
<div class="modal fade" id="dispatchModal" aria-hidden="true">
         ...
         <div class="modal-body">
            ...
            <form id="dispatchForm" class="row g-3">
               <div class="col-12">
                  ...
                  <input type="hidden" name="dispatchedItemId" id="dispatchedItemId" >
                  <div class="item_id error"></div>
               </div>
               <div class="col-12">
                  <label class="form-label" for="customer_id">Customer</label>
                    <select name="customer_id" id="customer_id" class="form-select"  placeholder="Selecet Item">
                        @foreach($customers as $key=> $customer)
                            <option value="{{ $customer->id}}">{{$customer->name }}</option>
                        @endforeach
                    </select>
                  <div class="customer_id error"></div>
               </div>
...
<script>
                    ...
                    columns: [
                        ...
                        @canany(['edit-item', 'delete-item'])
                        {
                            ...
                            "render": function(data, type, row) {
                                ...
                                @can('edit-item')
                                    actionButtons += `<a href="javascript:void(0);" class="editDispatchedItem btn btn-sm btn-warning mx-1" data-id="${row.id}">Edit</a>`;
                                @endcan
                                
                                @can('delete-item')
                                    actionButtons += `<a href="javascript:void(0)" class="deleteDispatchedItem btn btn-sm btn-danger" data-id="${row.id}">Delete</a>`;
                                @endcan
                                actionButtons += '</div>';
    ...
    $('#addDispatch').click(function (){
        ...
        $('.modalTitle').text('Add Dispatch Item');
        $('input').val('');
        $('#submit').val('Submit');
    ...
    $('#dispatchForm').validate({
            ...
            submitHandler: function(form){
                ...
                var url = '';
                if(type =='Submit'){
                    url = "{{ route('storeDispatchedItem') }}";
                }else{
                    url = "{{ route('updateDispatchedItem') }}";
                }
    ...
    $(document).on('click', '.editDispatchedItem', function() {
        ...
        let dispatchedItemId = $(this).data('id');
        $('#submit').val('Update');
        $('#submit').text('Update');
        var url = '/editDispatchedItem/'+dispatchedItemId;
        $.ajax({
            url : url,
            method: "GET",
            success: function(response){
                ...
                $('#dispatchModal #date').datepicker('update', date);

                var itemIdToSelect = response.item_id;
                var customerIdToSelect = response.customer_id;

                $('#item_id').val(itemIdToSelect).trigger('change');
                $('#customer_id').val(customerIdToSelect).trigger('change');
                $('#dispatchModal #quantity').val(response.quantity);
                $('#dispatchModal #dispatchedItemId').val(response.id);
    ...
    $(document).on('click', '.deleteDispatchedItem', function() {

        id= $(this).data('id');
        var urlToRedirect = 'deleteDispatchedItem/'+id;  
