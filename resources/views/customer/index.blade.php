                        ...
                        @can('add-item')
                            <div>
                                <a href="javascript:void(0);">
                                    <button class="btn btn-primary" data-bs-target="#customerModal" id="addItem">Add Customer</button>
                                </a>
                            </div>
                        @endcan
                    </div>
         ...
         <div class="modal-body">
            ...
            <form id="customerForm" class="row g-3">
               ...
               <div class="col-12 text-center">
                  <button type="submit" id="submit" class="btn btn-primary me-sm-3 me-1 sm-me-0 my-2">Submit</button>
...
<script>
    ...
    $(document).on('click', '.editCustomer', function() {
        ...
        $('#submit').val('Update');
        $('#submit').text('Update');
        var url = '/editCustomer/'+userId;
        $.ajax({
            url : url,
            method: "GET",
            success: function(response){
                console.log(response);
                $('#customerModal #name').val(response.name);
                $('#customerModal #email').val(response.email);
                $('#customerModal #mobile').val(response.mobile);
                $('#customerModal #address').val(response.address);
                $('#customerModal #customerId').val(response.id);
            }
        })
    });
    ...
    $(document).on('click', '.deleteCustomer', function() {
        ...
        id= $(this).data('id');
        var urlToRedirect = 'deleteCustomer/'+id;  
